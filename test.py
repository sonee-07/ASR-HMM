...
def get_features(data):
	...
	for i in range(0,len(data)):
		object=data['features'][i].tolist()
		features.append(object)
	feature_1=pd.DataFrame([subject for subject in features])
	return feature_1

def classify(models,models_w,test_data,col):
	...
	for i in range(0,len(mixture_result['uh'])):
		...
		for k in unique_labels:
			if(max_prob<(mixture_result[k])[i]):
				...
				max_label=k
		file_1=test_mfcc_region[i]+"_"+test_mfcc_speaker[i]+"_"+test_mfcc_sentence[i]
		if(max_label==test_mfcc_labels[i]):
			
			if(file_1 in pers):
				(pers[file_1])['count']=(pers[file_1])['count']+1
			else:
				pers[file_1]={}
				(pers[file_1])['count']=1
				(pers[file_1])['total']=0
		...
		if(file_1 in pers):
			(pers[file_1])['total']=(pers[file_1])['total']+1
		else:
			pers[file_1]={}
			(pers[file_1])['total']=1
			(pers[file_1])['count']=0
	...
	pers={}
	for phoneme in unique_labels:
		scores=(models_w[phoneme]).score_samples(test_data.drop(cols=col,axis=1))
		mixture_result[phoneme]=scores
	for i in range(0,len(mixture_result['uh'])):
		max_label='sil'
		max_prob=-float('inf')
...
for mixture in mixture_list:
	...
	pers={}
	for phoneme in unique_labels:
		scores=(mfcc_models_w[phoneme])[mixture].score_samples(test_mfcc.drop(cols=[0],axis=1))
		mixture_result[phoneme]=scores
	for i in range(0,len(mixture_result['uh'])):
		max_label='sil'
		max_prob=-float('inf')
