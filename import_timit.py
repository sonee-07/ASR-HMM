...
def _preprocess_data(args):
    ...
    for root, dirnames, filenames in os.walk(target):
        for filename in fnmatch.filter(filenames, "*.WAV"):
            ...
            if("TEST" in wav_file):
                wavs_test.append(wav_file)
            else:
                wavs_train.append(wav_file)
            if(~preprocessed):
	...
	labels = []
        ...
        sentence=[]
        dir_list=wav.split("/")
        trans, durations = read_transcript(wav = wav)
        n_delta = int(args.n_delta)
        
        (sample_rate,wav_file) = wav.read(wav)
        mfcc_feats = compute_mfcc(wav_file[durations[0][0]:durations[0][1]], n_delta=n_delta)
        for i in range(len(mfcc_feats)):
            labels.append(trans[0])
            reg.append(dir_list[-3])
        ...
        for index, chunk in enumerate(durations[1:]):
            ...
            for i in range(len(mfcc_feat)):
                ...
                speaker.append(dir_list[-2])
                sentence.append((dir_list[-1].split("_"))[0])
        mfccFeatTest.extend(mfcc_feats)
        mfccLabelTest.extend(labels)
        mfccRegTest.extend(reg)
        mfccSpeakerTest.extend(speaker)
	mfcc_sentence_test.extend(sentence)
...
if __name__ == "__main__":
    ...
    parser.add_a	rgument('--n_delta', type=str, default="0",
